...
|---|---|---|---|
| test-ui | mandatory | ✅ | Storybook 10 — изолированная разработка + визуальные регрессии + a11y-аудит |
...
npm install -D storybook @storybook/svelte-vite \ @storybook/addon-a11y @storybook/addon-themes @storybook/test-runner serve
 @storybook/addon-a11y @storybook/addon-themes \
 @storybook/test-runner serve
...
### D-IU01 — Storybook 10 как test-ui (ядро включает essentials, interactions, blocks, test)

- **Status:** active
- **Recorded:** session Discovery, infra-ui
- **Why:** Storybook 10 — индустриальный стандарт для изолированной разработки UI-компонентов. Нативная интеграция с Svelte 5 через `@storybook/svelte-vite`. Встроенный a11y-аудит (axe-core), interaction testing (play-функции), theme switching. Vite-совместимость — общий кэш трансформаций с dev-сервером проекта.
...
|---|---|---|---|
| Storybook packages | package | this-scope-task | `npm install -D storybook @storybook/svelte-vite @storybook/addon-a11y @storybook/addon-themes @storybook/test-runner @storybook/test` |
...
- **Open risks:**
 ...
 2. **Storybook 10 + Svelte 5** — адаптер `@storybook/svelte-vite` должен поддерживать runes ($state, $derived, $effect). Проверить совместимость на старте bootstrap.
 ...
 9. **Storybook 10 HMR + Svelte 5 runes** — `$state` / `$derived` изменения могут не подхватываться HMR Storybook (компиляторная реактивность вместо Proxy-based). Compensating action: проверить на первом bootstrap-тикете; при проблемах — `storybook dev --force-build-preview` или полная перезагрузка.
